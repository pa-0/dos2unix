...
B<dos2unix>: In this mode DOS line breaks are converted to Unix line breaks.
Unix and Mac line breaks are not changed.

B<unix2dos>: In this mode Unix line breaks are converted to DOS line breaks.
DOS and Mac line breaks are not changed.
...
B<dos2unix>: In this mode Mac line breaks are converted to Unix line breaks.
DOS and Unix line breaks are not changed. You can also use the command C<mac2unix> to
run dos2unix in Mac mode.

B<unix2dos>: In this mode Unix line breaks are converted to Mac line breaks.
DOS and Mac line breaks are not changed. You can also use the command C<unix2mac> to
run unix2dos in Mac mode.

=item B<7bit>

In this mode all 8 bit non-ASCII characters (with values from 128 to 255) are
converted to a space. 7bit mode can be combined with B<ascii> and B<mac> mode.
...
Iso mode can be combined with B<ascii> and B<mac> mode.
...
files can have DOS, Unix or Mac line breaks. It is safe to run dos2unix/unix2dos on
